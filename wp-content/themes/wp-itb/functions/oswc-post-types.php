	...
	function __construct($options){
		...
		if(!isset($this->post_type_vars)){
			$this->post_type_vars = array(
				'labels' => array(
					'name' => ucwords($this->name) . __( ' Articles' , 'itb'),
					'singular_name' => ucwords($this->name) . __( ' Article' , 'itb'),
					'add_new' => __('Add new article', 'itb'),
					'edit_item' => __('Edit article', 'itb'),
					'new_item' => __('New article', 'itb'),
					'view_item' => __('View article', 'itb'),
					'search_items' => __('Search articles', 'itb'),
					'not_found' => __('No articles found', 'itb'),
					'not_found_in_trash' => __('No articles found in Trash', 'itb')
				),
				'public' => true,
				'menu_position' => 27,
...
class oswcPostTypes {
			...
			foreach($this->postTypes as $postType){
				if($postType->enabled) {
					...
					register_sidebar(array(
						...
						'description'   => __( 'These widgets appear in the right sidebar of the ' . $postType->name . ' reviews pages.', 'itb' ),
					...
					register_sidebar(array(
						...
						'description'   => __( 'These widgets appear to the right of the featured slider on the ' . $postType->name . ' review listing page.', 'itb' ),
						'before_widget' => '<div class="widget-wrapper"><div class="widget">',
						'after_widget'  => '</div></div>',
						'before_title'  => '<div class="section-wrapper"><div class="section">',
						'after_title'   => '</div></div>'
	 ...
	 * Method: has_type
	 *
	 * A utility function to determine if itb has a post type named "n",
	...
	public function the_taxonomies($postType, $getExcerptOnly = NULL){
		...
		foreach($taxonomies as $taxonomy){
			echo get_the_term_list( $post->ID, $taxonomy->id, '<span class="taxName">' . __(ucwords($taxonomy->name),'itb') . '</span>: <span class="taxContent">', ', ', '</span>' );
